...
const RocketSvg = styled(Svg)`
  ...
  animation: calc(max(var(--duration), 10s) * 3) cubic-bezier(0.12, 0, 0.39, 0) 0s infinite
    move_rocket;
`;
  ...
  useEffect(() => {
    const updatePath = () => {
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        if (entry === "C") {
          const [x1, y1, x2, y2, x, y] = mapPoints(points.slice(i + 1, i + 7).map(parseFloat));
          newPath += `${entry} ${x1} ${y1} ${x2} ${y2} ${x} ${y}`;
          i += 6;
        }
      }

      window.document.documentElement.style.setProperty("--wave-path", `"${newPath}"`);
    };
...
const LanderSvg = styled(Svg)`
  left: calc(25vw - 106px / 2);
  top: calc(-64px - 150px); /* 64px is the padding on Main */
