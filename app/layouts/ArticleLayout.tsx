...
import MDXContent, { type MDX } from "components/MDXContent";
...
const ArticleLayout = ({ mdx, toc = false }: { mdx: MDX; toc?: boolean }) => {
  const headings = [
    {
      id: "wow",
      text: "wow",
      ...
      <HeroWrapper>
        ...
        <Hero>
          <MaxWidthWrapper>
            ...
            <Title>{mdx.frontmatter.title && capitalize(mdx.frontmatter.title)}</Title>
            <Subtitle>
              {mdx.frontmatter.date && formatDate(new Date(mdx.frontmatter.date))}
            </Subtitle>
            {mdx.frontmatter.tags.length > 0 && (
              <Tags>
